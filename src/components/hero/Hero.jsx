...
import forrestor from '../../assets/forrester.png';
import bannerWithoutBg from '../../assets/banner-png.png';
        ...
        <div className="w-2/5 flex flex-col max-xl:w-3/4 max-sm:w-full">
          ...
          <div className="flex gap-2 items-center mt-6 max-sm:flex-col max-sm:gap-6">
            <input
              ...
              placeholder="Email"
            />
            <button className="uppercase bg-action py-4 text-white rounded-sm text-md w-2/5 max-sm:w-full max-sm:py-3">
              let's try it out!
            </button>
          </div>
        </div>
        <div className="w-2/4 relative max-xl:w-3/4 max-lg:w-full">
          <img src={bannerWithoutBg} className="absolute w-full z-30 flex pointer-events-none"/>
          <img src={banner} />
          <Tag text={'Password policy applied'} className={'bg-[#fdd598] absolute top-20 left-10 z-20 flex max-xl:left-16 max-sm:top-2 max-sm:left-2'} color={"text-[#fdd598]"}/>
          <Tag text={'TvOS Kiosk Active'} className={'bg-[#c3e1e7] absolute top-48 right-[-4%] z-20 flex max-xl:right-0 max-sm:top-20 max-sm:right-0'} color={"text-[#c3e1e7]"}/>
          <Tag text={'Device Enrolled'} className={'bg-[#bcdcbe] absolute bottom-14 right-44 z-40 flex max-sm:bottom-4 max-sm:w-fit max-sm:right-20'} color={"text-[#bcdcbe]"}/>
        </div>
      </div>
        ...
        <div className="flex w-container m-auto py-8 max-sm:flex-col max-sm:gap-6">
          {partners.map((partner) => (
            <div className="flex flex-col gap-4 w-2/4 border-r border-r-divider first:pl-0 last:border-r-transparent px-20 justify-start max-xl:px-10 max-lg:w-full max-sm:justify-center max-sm:items-center max-sm:border-r-transparent max-sm:p-0 max-sm:border-b max-sm:border-b-divider max-sm:last:border-b-transparent max-sm:py-6">
              <img src={partner.image} className="contain" width={partner.width} height={25}/>
